...
describe("TournamentDetailView", () => {
  it("renders tournament header and tab labels", () => {
    ...
    expect(screen.getByRole("heading", { name: "Puchar testowy" })).toBeInTheDocument();
  ...
  it("shows empty copy for upcoming when there are no upcoming games", () => {
    ...
    expect(screen.getByText("Brak nadchodzących meczów.")).toBeInTheDocument();
  });
  ...
  it("lists upcoming games when active tab is upcoming", () => {
    ...
    expect(screen.getByTestId("game-game_u1")).toHaveTextContent("Gospodarze — Goście");
  });
  ...
  it("shows empty copy for finished when there are no finished games", () => {
    ...
    expect(screen.getByText("Brak zakończonych meczów.")).toBeInTheDocument();
  });
