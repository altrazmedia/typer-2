...
describe("BetForm", () => {
  ...
  it("shows a placeholder score when there is no saved bet", () => {
    render(
      <BetForm gameId="g1" homeTeam="Dom" awayTeam="Goście" userBet={null} />,
    );
    expect(screen.getByRole("button", { name: /Twój typ: \? - \?/ })).toHaveTextContent("? - ?");
  });
  ...
  it("shows the saved bet scores on the trigger", () => {
    ...
    expect(screen.getByRole("button", { name: /Twój typ: 1 - 2/ })).toHaveTextContent(
      "1 - 2",
    );
  });

  it("saves when the popover is closed after both scores are chosen", async () => {
    render(<BetForm gameId="g1" homeTeam="Dom" awayTeam="Goście" userBet={null} />);
  ...
  it("shows a toast when saving fails after closing", async () => {
    ...
    await waitFor(() =>
      expect(toast.showErrorToast).toHaveBeenCalledWith("Nie udało się zapisać zakładu."),
    );
  });
});
